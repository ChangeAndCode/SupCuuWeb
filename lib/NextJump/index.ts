 ...
 */
export async function getNextJumpData(culture: string = "en-us"): Promise<NextJumpProps> {
  const data = (await getUmbracoContent("/sites/nextjump/", culture)) as NextJumpPage;

  // Header Composition Properties
  const principalText =
    data?.properties?.principalText?.items.map((item: any) =>
      getText(item)
    ) || [];
  const subtext =
