...
const Notification = dynamic(() => import("../components/Notification"), {
  loading: () => (
    <div className="h-[48px] animate-pulse bg-gray-200/20 rounded-lg" />
  ),
});
...
export default function ClientForm() {
  ...
  const { formData, isSubmitting, status, handleChange, handleSubmit } =
  ...
  const topicsConfig =
    config.customContactFormFields.items[0]?.content.properties;
  ...
  return (
    <div className="relative bg-ColorPrincipal rounded-t-7xl px-[2rem] md:px-[3rem] lg:px-0 pt-[10rem] z-30">
      ...
      <div className="flex flex-col items-center">
        <form
          ...
          className="space-mt-8 w-full md:w-10/12 lg:w-8/12 xl:w-7/12 2xl:w-6/12"
        >
          <InputField
          ...
          >
            {isSubmitting
              ? config.sendingSendButtonText
              : config.defaultSendButtonText}
          </button>
        </form>
      </div>
    </div>
