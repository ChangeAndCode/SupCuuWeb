...
import { resultCarousels } from "@/types/home";

interface Props {
  carousels: resultCarousels;
}
const TransCarousel: React.FC<Props> = ({ carousels }) => {
  return (
    <>
      {carousels.items.map((carousel, index) => (
        <div key={index} className="flex flex-col items-center">
          <h3 className="text-[2rem] font-PerformanceMark text-ColorPrincipal">
            {carousel.content.properties.resultCarouselTitle}
          </h3>
          <div className="space-y-6">
            {carousel.content.properties.resultCarousel.items.map(
              (item, idx) => (
                <div key={idx}>
